...
public class DataBasePublisher {
    ...
    static String jdbc_insert_sql_industry = "INSERT INTO " + DataBaseName.industrytype + "(Industry) VALUES" + "(?)";
    ...
    public void addStockTable(PreparedStatement sql_statement_stock, StockReport stockReport) {
        ...
        if (!StockClassifier.stockPresent(symbol)) {
            try {
                ...
                PreparedStatement preparedStatement = (PreparedStatement) con.prepareStatement("select * from " + DataBaseName.industrytype);
                sql_statement_stock.setString(1, symbol);
                sql_statement_stock.setString(2, name);
                sql_statement_stock.setInt(3, IndustryClassifier.fetchUp(preparedStatement, stockReport.getIndustry()));
                sql_statement_stock.execute();
    ...
    public void recovery(PreparedStatement sql_statement) {
        IndustryClassifier.setUp(sql_statement);
